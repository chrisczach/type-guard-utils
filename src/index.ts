...
export const typeGuardOperatorFactory = <Operator extends <T>(...args: any[]) => MonoTypeOperatorFunction<T>>(
  operator: Operator
) => <T>(
  ...
  operator(
    handleTypeGuardValidation<T>(typeChecker, expectArray, invalidTypeCallback, validTypeCallback)
  );
...
export const checkWithTypeGuard = (
  ...
  const checkValue = handleTypeGuardValidation(typeChecker, expectArray, invalidTypeCallback, validTypeCallback)
  ...
  descriptor.value = function typeGuardWrapper<T>(...args: T[]) {
    ...
    if (isObservable) {
      return result.pipe((mapInsteadOfCheck ? typeGuardMapOperator : typeGuardCheckOperator)(typeChecker, expectArray, invalidTypeCallback, validTypeCallback));
    }
    if (isPromise) {
